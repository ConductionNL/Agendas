...
The Helm chart installs agendaservice and by default the following dependencies using subcharts:
...
```bash
helm repo add agendaservice https://raw.githubusercontent.com/ConductionNL/agendaservice/master/api/helm/
helm repo update
```
...
```bash
helm install my-agendaservice ConductionNL/agendaservice --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `agendaservice` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `agendaservice` |
